...
This project implements the deployment of the provided spam detection web application and its associated spam classification model within a Kubernetes-based environment. This document reports on the final deployment structure as well as the end-to-end flow of the system.
...
A Kubernetes Deployment manages a set of identical Pods and keeps the application running in the desired state. With this, pods can be automatically created, restarted and replaced.
...
As mentioned in the overview, Istio extends the Kubernetes deployment with a service mesh. The mesh is an infrastructure layer that manages communication between services in a distributed system. This would enable advanced traffic control, observability and so on, without having to modify any application code. 
...
<p align="center">
  <img src="./images/traffic_management_flow.drawio.png" alt="traffic_management" width="800"><br>
  <em>Basic traffic management</em>
</p>
...
<p align="center">
  <img src="./images/rate_limiting.drawio.png" alt="rate_limiting" width="800"><br>
  <em>Rate limiting</em>
</p>
...
- Prometheus: This tool collects metrics from an exposed port (/metrics) in app. This looks at all the metrics defined in FrontendController, and generally looks at application behaviour and request traffic. It scrapes via a ServiceMonitor - a resource that enables Prometheus to actually discover the exposed metrics.  
- Grafana: This tool is able to provide visualizations tuned to whatever metrics it gets from Prometheus. This enables the analysis of these metrics, and allows for the comparison of the different deployment versions.
...
- Alerting: A PrometheusRule was used to define an alert condition and AlertManager was used to send the email when the alert is triggered. 

<p align="center">
  <img src="./images/monitoring.drawio.png" alt="monitoring" width="800"><br>
  <em>Monitoring</em>
</p>
...
2. The request is received by the Istio Ingress Gateway and then forwards it to the service mesh.
3. The Istio Gateway then receives the request and makes the decision whether to allow the request in.
4. When it enters the service mesh, the request is then evaluated against Istio VirtualService rules. This will apply traffic routing logic (splitting and sticky sessions). It also references the DestinationRule to get the appropriate subset. 
3. The request is then routed to the frontend Kubernetes Service.
4. The frontend Service will forward the request to one of the frontend pods listening on port 8080.
...
7. The backend returns the result of the model to the frontend, which then displays it to the user. 
